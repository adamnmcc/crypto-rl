...
def sma(trial: optuna.Trial, i: int, column: str = None) -> Dict[str, Any]:
    if not column:
        column = trial.suggest_categorical(f"column_{i}", ["close", "open", "high", "low"])
    
    period_length = trial.suggest_int(f"period_length_{i}", 5, 100, log=True)    
    return f"{column}_{int(period_length)}_sma"


def rsi(trial: optuna.Trial, i) -> Dict[str, Any]:
    rsi = trial.suggest_int(f"rsi_{i}", 1, 100, log=True)
    return f"rsi_{int(rsi)}"
